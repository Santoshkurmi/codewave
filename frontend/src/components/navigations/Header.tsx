...
function Header({header}:any) {
  ...
  return (
    <div 
    className={"h-[5rem] transition-['top'] z-50 bg-white dark:bg-black fixed left-0  right-0 select-none shadow-sm dark:shadow-slate-400 rounded-md flex items-center justify-between px-0 lg:px-8 "+ (header? '-top-[6rem]':"top-0")}>
        ...
        <div className="right text-gray-700 justify-center items-center flex gap-5 pr-5 ">
            ...
            <FontAwesomeIcon onClick={()=>setTheme(!isDarkTheme)} className="cursor-pointer  dark:text-slate-300 dark:hover:text-slate-500  dark:active:text-slate-100 rounded-full"  icon={faMoon} size="2x" />
